...
```bash
mkdir -p ~/SANDBOX/Noble #my custom name, you can put whatever you want
cd ~/SANDBOX
sudo -s
...
Since you want  to play around with this chroot for a while, you maybe want o make these mounts persistent during reboot, add them to `/etc/fstab`:
